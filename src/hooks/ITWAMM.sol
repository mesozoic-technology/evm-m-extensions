...
interface ITWAMM {
    ...
    function getOrder(PoolKey calldata poolKey, OrderKey calldata orderKey) external view returns (Order memory order);
    ...
    ) external returns (uint256[] memory tokensClaimed);
    ...
    function claimTokensByCurrencies(Currency[] calldata currencies) external returns (uint256[] memory tokensClaimed);
