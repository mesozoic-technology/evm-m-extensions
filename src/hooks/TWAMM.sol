 ...
 * @author Zaha Studio
 */
contract TWAMM is BaseHook, Ownable, ITWAMM {
    ...
    function getOrderPool(
        PoolKey calldata key,
        bool zeroForOne
    ) external view returns (uint256 sellRateCurrent, uint256 earningsFactorCurrent) {
    ...
    function executeTWAMMOrders(PoolKey memory key, uint256 targetTimestamp) public {
        ...
        if (sqrtPriceLimitX96 != 0 && sqrtPriceLimitX96 != sqrtPriceX96 && maxSwapAmount != 0) {
            SwapParams memory swapParams = SwapParams(zeroForOne, -maxSwapAmount.toInt256(), sqrtPriceLimitX96);
    ...
    ) external returns (bytes32[] memory orderIds, OrderKey[] memory orderKeys) {
        ...
        )
    {
        ...
        buyTokensOwed =
            (Math.mulDiv(earningsFactorLast - order.earningsFactorLast, order.sellRate, RATE_SCALER)) >>
    ...
    function _unlockCallback(bytes calldata rawData) internal returns (bytes memory) {
        (PoolKey memory key, SwapParams memory swapParams) = abi.decode(rawData, (PoolKey, SwapParams));
    ...
    function _executeTWAMMOrders(
        ...
        if (
            currentTimestampAtInterval > block.timestamp || currentTimestampAtInterval < self.lastVirtualOrderTimestamp
        ) {
    ...
    function _advanceTimestampForSinglePoolSell(
        ...
        params.zeroForOne = _exhaustMatchedOrders(
            self,
            AdvanceParams(expirationInterval, params.nextTimestamp, params.secondsElapsed, params.pool)
        );
        ...
        while (true) {
            ...
            (bool crossingInitializedTick, int24 tick) = _isCrossingInitializedTick(
                params.pool,
                poolKey,
                finalSqrtPriceX96
            );
            ...
            if (crossingInitializedTick) {
                ...
                uint256 accruedEarningsFactor = Math.mulDiv(
                    totalEarnings * FixedPoint96.Q96,
                    RATE_SCALER,
                ...
                self.orderPool0For1.advanceToInterval(
                    ...
                    params.zeroForOne ? accruedEarningsFactor : 0
                );
                self.orderPool1For0.advanceToInterval(
                    params.nextTimestamp,
                    params.zeroForOne ? 0 : accruedEarningsFactor
                );
